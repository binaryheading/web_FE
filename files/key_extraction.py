...
class keywordExtractor:
    ...
    def __init__(self, model=None, tokenizer=None, dir: str = None) -> None:
        ...
        self.dir = dir if dir else "../../content/eng_han.csv"
    ...
    def extract_keyword_list(self, doc: pd.Series, min_count: int = 2, min_length: int = 2) -> List:
        """
    ...
    def _convert_series_to_list(self, series: pd.Series) -> List[str]:
        """Series에 속한 'name', 'category', 'description' 값을 단어 리스트로 변환"""
        # 'name', 'category', 'description' 열의 값을 각각 리스트로 변환합니다.
        food_name = series['name'].split()
        food_category = series['category'].split()
        description = series['description'].split()
        # 세 리스트를 연결합니다.
        raw_data = food_name + food_category + description
        return raw_data
    ...
    def _eliminate_min_count_words(self, candidate_keyword, min_count: int = 2):
    ...
    def _convert_series_to_str(self, series: pd.Series) -> str:
        ...
        food_name = series["name"]
        series = series.drop(["name"])
        return food_name+ " " + " ".join(list(chain(*series.values)))
    ...
    def extract_keyword(self, docs: pd.DataFrame) -> Dict:
        """
        음식 데이터를 기반으로 키워드를 추출하는 메서드입니다.
        클래스 내 create_keyword_list, create_keyword_embedding, create_doc_emeddings를 기반으로 동작합니다.

        Parameter
        ---------
        - docs : pd.DataFrame 타입의 데이터이며 column은 [name, category, description]이어야 합니다.

        """
        if docs.columns.tolist() != ["name", "category", "description"]:
            raise ValueError(
                f"{docs.columns.tolist()} doesn't match with ['name', 'category', 'description']"
            )
        ...
        return dict(name=docs["name"].tolist(), keywords=top_n_keyword)
